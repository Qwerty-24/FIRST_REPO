...
private:
    ...
    void sink(int index, int heapSize) {
        ...
        int mx_idx = index, mx = array[index];
        int lc = leftChild(index), rc = rightChild(index);
        if (lc >= heapSize && rc >= heapSize){
            return;
        }
        if (lc < heapSize && array[lc] > mx){
            mx_idx = lc, mx = array[lc];
        }
        if (rc < heapSize && array[rc] > mx){
            mx_idx = rc;
        }
        if(mx_idx != index){
            swap(array[mx_idx], array[index]);
            sink(mx_idx, heapSize);
        }
        return;
    }
